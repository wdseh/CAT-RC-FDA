...
class ICRm:
    def __init__(self,num_classes = 20, max_save = 50, dir = "", blocked_classes = [],mix_ratio=0.5, cfg = None):
    ...
    def mix_crop_new(self,label_data,target=False):
        ...
        for data in label_data:
            ...
            #     for box, cls in zip(data['instances'].gt_boxes, data['instances'].gt_classes):
            #     # 生成随机颜色
            ...
            #     cv2.imwrite('./image_crop/label_data/' + str(self.itera_tim) + '.jpg', image_np)
            #     self.itera_tim += 1  
